...
static void fmt_function_header(DCFormatterContext *ctx, char *dst, size_t n, DCLangRoutine *il_routine)
{
    ...
    for (DCLangVariable *v = il_routine->variables; v; v = v->next)
        if (v->is_param) {
            ...
            DC_FormatAppend(dst, n, ": i%d", v->size);
    ...
    for (DCLangVariable *v = il_routine->variables; v; v = v->next)
        if (!v->is_param) {
            ...
            DC_FormatAppend(dst, n, "var ");
            DC_FormatAppendVariable(ctx, dst, n, v);
            DC_FormatAppend(dst, n, ": i%d = undefined;\n", v->size);
        }
...
{
    /* stub */
    DC_FormatAppend(dst, n, "mem_read(%s, %d)", location_expr, bitsize / 8);
}

DCFormatterContext DC_FormatterLangZig()
{
